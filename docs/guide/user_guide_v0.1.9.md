...
On subsequent runs, you simply enter the passphrase you chose and press Enter. Once you have signed in, the browser's back button will not return you to the passphrase page — you stay in the application until you choose to sign out.
...
When you sign in, the home page welcomes you with a brief description of what Portfolio 60 does. Alongside the welcome text you will see a row of screenshot thumbnails showing key screens in the application — the portfolio summary, analysis charts, reports and more. Click any thumbnail to open a full-size lightbox view, then use the arrow buttons or your keyboard to browse through the screenshots as a carousel.
...
Sometimes an investment is replaced by a new one due to a corporate action — a fund restructure, a merger or an acquisition. When this happens, you need to close the old holding and open a new one with the replacement investment.

Portfolio 60 handles this through the **Replace** option on the Edit Holding screen.

### How to replace a holding

1. Navigate to the holding you want to replace (via Portfolio Setup or Account Detail)
2. Click **Edit** on the holding
3. Tick the **Replace** checkbox — additional fields will appear
4. Select the **replacement investment** from the dropdown
5. Enter the **new quantity** — the number of units or shares you received in the replacement
6. Enter the **new price** — the price per unit of the replacement investment
7. Enter the **effective date** — the date the replacement took effect
8. Click **Save**

The application will close the old holding as of the effective date and create a new holding for the replacement investment with the quantity and price you specified. The old holding remains visible in your historic holdings for record-keeping purposes.

This is the correct way to handle fund restructures and similar corporate actions — it preserves an accurate history of what you held and when, rather than simply editing the existing holding.
...
If you have configured the companion fetch-server-60 service (an always-on server that fetches prices independently), a log viewer is available on the Fetching page. This shows the remote server's fetch history, so you can check that the fetch server is running correctly and see when it last fetched prices.
...
Three filters appear beneath the page title:
...
- **Users** — a dropdown to select which family member's holdings to include. Choose *All users* to see everyone's investments together, or select an individual user to focus on their holdings alone.

- **Account type** — filter by account type: *All*, *ISA*, *SIPP* or *Trading*. This lets you see, for example, only the investments held in ISA accounts. This filter is only available when a specific user is selected — when *All users* is chosen, the account type filter is disabled.
...
The **Reports Manager** is the primary way to create, edit and organise your reports. You can access it from **Settings > Reports Manager** or from **Reports > Manage Reports** in the menu bar.

The Reports Manager provides a visual interface for:

- Creating new reports and choosing which report blocks to include
- Editing existing reports — changing their title, layout and content
- Reordering and deleting reports
- Previewing reports before generating them

For advanced users, reports can also be edited directly as JSON configuration — but the Reports Manager is the recommended approach for most users.
...
The Lists Manager lets you organise two types of external content alongside your portfolio: embedded spreadsheets and uploaded documents.

### Spreadsheets

The Spreadsheets section lets you add links to online spreadsheets — for example, a Google Sheet or Excel Online document that you maintain separately. This is useful for data you keep in a spreadsheet and want to view alongside your portfolio, such as a list of non-SIPP pensions or insurance policies.

For each spreadsheet, enter a title and the sharing URL. The spreadsheet will appear as an embedded view in the Lists menu. You can edit the title or URL at any time, reorder the list, or delete entries you no longer need.

### Documents

The Documents section lets you upload PDF files directly into Portfolio 60. You might use this for storing provider statements, tax documents, or any other PDF you want to keep handy.

To upload a document, either click the upload area or drag and drop a PDF file onto it. Uploaded documents appear in the Lists menu and can be viewed in the browser. You can delete documents you no longer need.
...
Also available from the **Docs** menu is **How Portfolio 60 Works** — an interactive course that explains how the application is built and how its different parts fit together. This is aimed at anyone who is curious about the technology behind Portfolio 60, or who might want to understand the codebase in more depth.
...
A read-only banner appears beneath the navigation bar confirming that you are in demonstration mode. The navigation bar itself remains the standard brand colour, so the banner is the visual indicator that you are browsing sample data rather than your own portfolio.
