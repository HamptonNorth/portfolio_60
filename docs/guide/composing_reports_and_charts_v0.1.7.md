...
## Reports Manager (Visual Editor)

Most users can now create, edit, reorder, duplicate and delete reports using the visual **Reports Manager** — no JSON editing required. You can access it from **Reports > Manage Reports** or from **Settings > Manage Reports**.

The Reports Manager lets you:

- **Create** new reports by choosing a report type and filling in a simple form.
- **Edit** existing reports by clicking on them in the list.
- **Reorder** reports by dragging them up or down — the order in the list matches the order in the Reports dropdown.
- **Duplicate** a report to use it as a starting point for a variation.
- **Delete** reports you no longer need.

The JSON editing approach described in the rest of this guide is still available as an advanced option for users who prefer direct control over the configuration file. Both approaches read and write the same `user-reports.json` file, so changes made in either place are immediately visible in the other.
...
Each entry in `params` must begin with either `inv:` or `bm:`. If you use any other prefix (or forget the prefix entirely), the system will show a clear error message when you save. Case is normalised automatically — for example, `INV:GB00B41YBW71` and `Bm:FTSE 100` are both accepted and will be corrected to lowercase prefixes.
...
**Invalid data series prefix:** Chart `params` entries must start with `inv:` or `bm:`. If you see a validation error on save, check that each entry uses one of these two prefixes. The system will automatically correct the case of the prefix for you, but the prefix itself must be present.
