...
let comparisonPeriods = ["3m", "6m", "1y", "3y"];

/** @type {number} Current comparison sort column index (0-3) */
let comparisonSortCol = 2;
...
async function loadBenchmarks() {
  try {
    ...
    const checkboxes = document.getElementById("benchmark-checkboxes");
    let html = "";
    ...
    const boxes = checkboxes.querySelectorAll(".bm-checkbox");
...
function handleBenchmarkChange() {
  const boxes = document.querySelectorAll(".bm-checkbox");
  const selected = [];
...
async function loadUsers() {
  try {
    ...
    for (let i = 0; i < result.data.length; i++) {
      ...
      const acctResult = await apiRequest("/api/users/" + user.id + "/accounts");
...
function setupHoldingsFilter() {
  const select = document.getElementById("holdings-filter");
  if (!select) return;
  ...
  select.addEventListener("change", function () {
    ...
    const userLabel = document.getElementById("user-filter-label-prefix");
    const accountGroup = document.getElementById("account-type-filter-group");
...
function setupAccountTypeFilter() {
  const select = document.getElementById("account-type-filter");
  if (!select) return;
...
async function loadAccountTypes() {
  ...
  try {
    const usersParam = selectedUserIds.join(",");
    const result = await apiRequest("/api/analysis/account-types?users=" + usersParam);
    if (!result.ok || !Array.isArray(result.data)) return;

    let html = '<option value="" selected>All</option>';
    for (let i = 0; i < result.data.length; i++) {
      const type = result.data[i];
...
function updateAccountTypeVisibility() {
  const group = document.getElementById("account-type-filter-group");
  const select = document.getElementById("account-type-filter");
...
function updateSortButtonStyles() {
  ...
  for (let i = 0; i < btns.length; i++) {
    ...
    if (key === leagueSort) {
      ...
      const arrow = leagueSortDir === "desc" ? " \u2193" : " \u2191";
...
function setupLeagueLimit() {
  const sel = document.getElementById("league-limit");
  if (!sel) return;
...
async function loadComparison() {
  ...
  const url = "/api/analysis/comparison?periods=" + comparisonPeriods.join(",") + benchmarksParam() + filterParams();
...
function renderComparisonTable(data) {
  ...
  const investments = data.investments.slice();
  const benchmarks = data.benchmarks || [];

  // Sort investments by the active sort column
  const sortCode = periods[comparisonSortCol] ? periods[comparisonSortCol].code : null;
  if (sortCode) {
    investments.sort(function (a, b) {
      const aVal = a.returns[sortCode];
      const bVal = b.returns[sortCode];
      // Nulls go to bottom
  ...
  let html = '<table class="w-full text-base">';
  ...
  for (let p = 0; p < periods.length; p++) {
    ...
    for (let ap = 0; ap < ALL_PERIODS.length; ap++) {
      const sel = ALL_PERIODS[ap] === periods[p].code ? " selected" : "";
  ...
  for (let b = 0; b < benchmarks.length; b++) {
    ...
    for (let bp = 0; bp < periods.length; bp++) {
      const bmVal = bm.returns[periods[bp].code];
  ...
  for (let i = 0; i < investments.length; i++) {
    ...
    const rowClass = i % 2 === 0 ? "bg-white" : "bg-brand-25";
    const nameHtml = buildResearchLinkHtml(inv.description, inv.publicId, inv.currencyCode, inv.morningstarId);
    ...
    for (let ip = 0; ip < periods.length; ip++) {
      const val = inv.returns[periods[ip].code];
  ...
  const selects = container.querySelectorAll(".comparison-period-select");
  ...
  const sortBtns = container.querySelectorAll(".comparison-sort-btn");
  for (let sb = 0; sb < sortBtns.length; sb++) {
...
async function loadLeagueTable() {
  ...
  const result = await apiRequest("/api/analysis/league-table?period=" + activePeriod + filterParams());
  if (!result.ok) {
    container.innerHTML = '<p class="text-error">' + escapeHtml(result.error) + "</p>";
    return;
  }
  ...
  if (selectedBenchmarkIds.length > 0) {
    const bmResult = await apiRequest("/api/analysis/risk-return?period=" + activePeriod + benchmarksParam());
...
function renderLeagueTable(data) {
  ...
  const byReturn = data.investments.slice().sort(function (a, b) {
    return b.returnPct - a.returnPct;
  });

  // Apply limit filter
  let filtered;
  const totalCount = byReturn.length;
  ...
  const investments = filtered.slice();
  investments.sort(function (a, b) {
    let val;
  ...
  let limitLabel = "";
  if (leagueLimit !== "all") {
    const limitMap = { top10: "Top 10", top20: "Top 20", bottom10: "Bottom 10", bottom20: "Bottom 20" };
  ...
  const bmRows = leagueBenchmarkData || [];
  for (let bm = 0; bm < bmRows.length; bm++) {
    const bmItem = bmRows[bm];
    const bmReturnClass = bmItem.returnPct >= 0 ? "text-green-700" : "text-red-600";
    const bmReturnSign = bmItem.returnPct >= 0 ? "+" : "";
    html += '<tr class="bg-brand-50 border-b border-brand-100">';
    html += '<td class="px-3 py-2 text-brand-400 text-sm"></td>';
  ...
  for (let i = 0; i < investments.length; i++) {
    const inv = investments[i];
    const rowClass = i % 2 === 0 ? "bg-white" : "bg-brand-25";
    const returnClass = inv.returnPct >= 0 ? "text-green-700" : "text-red-600";
    const returnSign = inv.returnPct >= 0 ? "+" : "";
    const nameHtml = buildResearchLinkHtml(inv.description, inv.publicId, inv.currencyCode, inv.morningstarId);
...
function renderSparkline(inv) {
  const canvas = document.getElementById("spark-" + inv.id);
  if (!canvas || !inv.sparkline || inv.sparkline.length < 2) return;

  const ctx = canvas.getContext("2d");
  const lineColour = inv.returnPct >= 0 ? "rgba(21, 128, 61, 0.8)" : "rgba(220, 38, 38, 0.8)";

  const labels = [];
  const dataPoints = [];
  for (let i = 0; i < inv.sparkline.length; i++) {
...
async function loadScatter() {
  ...
  const url = "/api/analysis/risk-return?period=" + activePeriod + benchmarksParam() + filterParams();
  const result = await apiRequest(url);
  if (!result.ok) {
  ...
  const bmCount = result.data.benchmarks ? result.data.benchmarks.length : 0;
  const bmLabel = bmCount > 0 ? " + " + bmCount + " benchmark" + (bmCount > 1 ? "s" : "") : "";
...
function renderScatter(data) {
  ...
  const points = data.investments.map(function (inv) {
    return { x: inv.volatility, y: inv.returnPct, label: inv.description };
  });

  // Calculate medians for quadrant lines
  const xValues = points.map(function (p) { return p.x; }).sort(function (a, b) { return a - b; });
  const yValues = points.map(function (p) { return p.y; }).sort(function (a, b) { return a - b; });
  ...
  const hasBenchmarks = data.benchmarks && data.benchmarks.length > 0;
  if (hasBenchmarks) {
    const bmPoints = [];
    for (let b = 0; b < data.benchmarks.length; b++) {
      const bm = data.benchmarks[b];
  ...
  scatterChart = new Chart(ctx, {
    ...
    plugins: [{
      ...
      afterDraw: function (chart) {
        ...
        const medXPx = xAxis.getPixelForValue(medianX);
        ctxDraw.beginPath();
        ctxDraw.moveTo(medXPx, yAxis.top);
        ctxDraw.lineTo(medXPx, yAxis.bottom);
        ctxDraw.stroke();

        const medYPx = yAxis.getPixelForValue(medianY);
        ctxDraw.beginPath();
...
async function loadTopBottom() {
  ...
  const url = "/api/analysis/top-bottom?period=" + activePeriod + "&count=5" + benchmarksParam() + filterParams();
  const result = await apiRequest(url);
  if (!result.ok) {
    document.getElementById("period-info").textContent = result.error;
    return;
  ...
  const bmSeries = d.benchmarkSeries || [];
...
const LONG_PERIODS = { "6m": true, "1y": true, "2y": true, "3y": true };
...
function getYearBoundaries(isoDates) {
  ...
  for (let i = 0; i < isoDates.length; i++) {
    const year = isoDates[i].substring(0, 4);
    if (year !== currentYear) {
...
const yearBandsPlugin = {
  ...
  beforeDraw: function (chart) {
    const boundaries = chart.options._yearBoundaries;
    ...
    for (let i = 0; i < boundaries.length; i++) {
      ...
      const endIdx = (i + 1 < boundaries.length) ? boundaries[i + 1].index : chart.data.labels.length - 1;
...
function renderPerformanceChart(canvasId, legendId, series, colours, bmSeries, isoDates) {
  const isLong = LONG_PERIODS[activePeriod] || false;
  const labels = isoDates.map(function (d) { return formatDateLabel(d, isLong); });
  ...
  for (let b = 0; b < bmSeries.length; b++) {
    ...
    const bmLabel = bm.label + (bm.returnPct !== null ? " (" + bmSign + bm.returnPct.toFixed(1) + "%)" : "");
    const bmStyle = getBenchmarkLineStyle(b, bmSeries.length);
    datasets.push({
      label: bmLabel,
      data: bm.values,
  ...
  const yearBoundaries = getYearBoundaries(isoDates);
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            afterTitle: function (items) {
              if (items.length > 0) {
                const dates = items[0].chart.options._isoDates;
  ...
  let legendHtml = '<div class="flex flex-wrap gap-4 text-xs">';
  for (let sl = 0; sl < series.length; sl++) {
    const ser = series[sl];
    const colour = colours[sl % colours.length];
    const linkHtml = buildResearchLinkHtml(ser.label, ser.publicId, ser.currencyCode, ser.morningstarId);
  ...
  for (let bl = 0; bl < bmSeries.length; bl++) {
    const bmColour = BENCHMARK_COLOURS[bl % BENCHMARK_COLOURS.length];
    const bmLegendStyle = getBenchmarkLineStyle(bl, bmSeries.length);
    const strokeW = Math.max(1, Math.round(bmLegendStyle.borderWidth));
    const dashAttr = bmLegendStyle.borderDash.join(",");
  ...
  for (let li = 0; li < legendItems.length; li++) {
    legendItems[li].addEventListener("click", function (evt) {
      ...
      if (isHighlighted) {
        ...
        for (let h = 0; h < chartInstance.data.datasets.length; h++) {
          if (h === dsIndex) {
        ...
        for (let ali = 0; ali < allLegendItems.length; ali++) {
