...
/**
 * @description Extract the exchange code from a ticker or ETF public_id.
 * For ticker format (EXCHANGE:TICKER, e.g. LSE:AZN), returns the exchange part (LSE).
 * For ETF format (TICKER:EXCHANGE:CURRENCY, e.g. ISF:LSE:GBX), returns the exchange part (LSE).
 * Returns null for ISINs, invalid input, or unrecognised formats.
 * @param {string} publicId - The public identifier
 * @returns {string|null} The exchange code, or null if not a ticker/ETF format
 */
export function extractExchangeFromPublicId(publicId) {
  if (!publicId || typeof publicId !== "string") {
    return null;
  }
